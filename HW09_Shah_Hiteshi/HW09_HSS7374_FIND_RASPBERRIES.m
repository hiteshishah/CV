% function that segments raspberries from the background using Mahalanobis
% distance
function HW09_HSS7374_FIND_RASPBERRIES( fn )
    ...
    % classifying raspberries as 1 if distance to foreground is < distance to background
    % with a tolerance of 2
    class_rasp     = mahal_fg < mahal_bg - 2;
    
    % calculating the mean and standard deviation of the foreground pixels
    fg_dists        = mahal_fg(class_rasp);
    ...
    better_class_rasp      = mahal_fg < dist_mean;
    
    % changing the shape of the classification to look like an image
    class_im        = reshape( better_class_rasp, size(im_a,1), size(im_a,2) );
